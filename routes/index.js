...
// authentication bridge from intranet
router.get('/auth_bridge', function(req, res) {
    var db = req.app.db;
    // set the template
    common.setTemplateDir('admin', req);
    ...
    var secret = options.auth_bridge_secret;

    if (!bcrypt2.compareSync(secret, token)) {
        console.error('Failed validating auth bridge token.');
        return res.redirect(config.settings.intranetBaseUrl);
    }

    // console.log("AUTH_BRIDGE");
    // console.log(`email: ${req.query.user}`);
    // console.log(`token: ${req.query.token}`);

    db.users.findOne({ user_email: email }, function(err, user) {
        // check if user exists with that email
        if (user === undefined || user === null) {
            // req.session.message = req.i18n.__('A user with that email does not exist.');
            // req.session.message_type = 'danger';
            res.redirect(config.settings.intranetBaseUrl);
        } else {
